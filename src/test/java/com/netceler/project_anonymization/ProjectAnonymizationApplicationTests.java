...
class ProjectAnonymizationApplicationTests {
    ...
    void should_return_a_file_when_get_url_is_called() throws Exception {
        final var defaultObject = new JSONObject().put("filename", "test.txt")
    ...
    void should_throw_exception_when_file_not_found() throws Exception {
        ...
        Assertions.assertThatThrownBy(() -> mockMvc.perform(get("/getFile/notfound.txt")))
    ...
    void should_throw_exception_when_file_cant_be_read() throws Exception {
        ...
        Assertions.assertThatThrownBy(() -> mockMvc.perform(get("/getFile/cantBeRead")))
                .isInstanceOf(BadRequestException.class)
    ...
    void should_save_uploaded_file_then_be_able_to_read_it() throws Exception {
        final var defaultObject = new JSONObject().put("filename", "test2.txt")
                .put("content", "text for test2")
                .toString();

        MockMultipartFile multipartFile = new MockMultipartFile("file", "test2.txt", "text/plain",
                "text for test2".getBytes());
