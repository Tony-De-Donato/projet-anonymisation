...
public interface DictionaryRepository extends JpaRepository<DictionaryEntity, Long> {
    ...
    @Query("select dictionary from DictionaryEntity dictionary where dictionary.fileName = ?1")
    List<DictionaryEntity> findByDictFileName(String dictFileName);

    @Query("select dictionary from DictionaryEntity dictionary where dictionary.fileName like %?1%")
    ...
    @Query("select dictionary from DictionaryEntity dictionary where dictionary.uniqueness = ?1 and dictionary.fileName = ?2")
